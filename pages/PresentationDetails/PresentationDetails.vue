<template>
	<view class="tabs">
		<scroll-view ref="tabbar1" id="tab-bar" class="tab-bar" :scroll="false" :scroll-x="true" :show-scrollbar="false"
		 :scroll-into-view="scrollInto">
			<view style="flex-direction: column;">
				<view style="flex-direction: row;"  class="flex flec-just-center">
		...
		<swiper class="tab-box bgf9" ref="swiper1" :current="tabIndex" :duration="300" @change="onswiperchange" @transition="onswiperscroll"
		 @animationfinish="animationfinish">
			<swiper-item class="swiper-item" v-for="(page, index) in tabList" :key="index">
				<!-- #ifndef MP-ALIPAY -->
				<listsPage class="page-item " :nid="page.newsid" ref="page"></listsPage>
				<!-- #endif -->
				<!-- #ifdef MP-ALIPAY -->
				<listsPage class="page-item  " :nid="page.newsid" :ref="'page' + index"></listsPage>
				<!-- #endif -->
			</swiper-item>
		</swiper>
	</view>
	...
	export default {
		...
		methods: {
			getData: function() {
				...
				_this.tui.request('/Dealer/getAgentInfo_h5').then(function(data) {

					if (data.stat == 1) {

						_this.tabList = [{
							id: "tab01",
							name: '我的直推（' + data.data.referee_num + '）',
							newsid: 0
						}, {
							id: "tab02",
							name: '我的团队（' + data.data.belong_num + '）',
							newsid: 1
						}]
...
<style scoped>
	...
	.flec-just-center{ justify-content: center;}
